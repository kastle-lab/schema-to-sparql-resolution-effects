...
def aggregate_evaluation_results(
    ...
    print(f"  [INFO] Aggregating results from {input_jsonl}...")
...
def evaluate_excel_directory(
    ...
    with open(output_jsonl, mode, encoding="utf-8") as out:
        for record in iter_excel_directory(input_directory, model_name):
            ...
            result = evaluate_excel_record(record, endpoint_by_graph=endpoint_by_graph, runs=runs, timeout=timeout)
    ...
    if output_csv:
        ...
        if rows:
            preferred = [
                "source_file", "line_number", "key", "task", "representation", "kg", "graph_id",
...
def evaluate_jsonl_file(
    ...
    with open(output_jsonl, mode, encoding="utf-8") as out:
        for record in iter_jsonl(input_jsonl):
            ...
            result = evaluate_jsonl_record(record, endpoint_by_graph=endpoint_by_graph, runs=runs, timeout=timeout)
            out.write(json.dumps(result, ensure_ascii=False) + "\n")
            out.flush()
            results.append(result)
            processed += 1
...
if __name__ == "__main__":
    ...
    # --------------------------------------------------------------------------
    # TOGGLE PIPELINE MODE HERE
    # Choices: "LOCAL_XLSX" or "PROPRIETARY_JSONL"
    # --------------------------------------------------------------------------
    RUN_MODE = "LOCAL_XLSX"  
    ...
    if RUN_MODE == "LOCAL_XLSX":
        # MODEL_NAMES = ["gpt-oss-120b", "qwen3.5-35b", "deepseek-r1-70b", "granite4.1-30b", "mistral-small3.2-latest", "qwen3.5-122b"]
        # MODEL_NAMES = ["mistral-small3.2-latest", "qwen3.5-122b"]
        MODEL_NAMES = ["mistral-small3.2-latest"]
    ...
    # --------------------------------------------------------------------------
        ...
        if not os.path.exists(input_file_path):
            ...
            print(f"          Please ensure {INPUT_FILENAME} is inside the '{INPUT_DIR}' folder.")
        else:
            evaluate_jsonl_file(
